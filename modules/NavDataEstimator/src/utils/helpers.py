...
def draw_distance_cloud(image, points_with_distances, detection_kernel_size):
    ...
    Args:
        ...
        points_with_distances: Dictionary with pos as keys and distances, bearing and bbox values.
    ...
    for (x, y), (distance, bearing, bbox_width, bbox_height) in points_with_distances.items():
        if detection_kernel_size == 0:
            kernel_half_width = bbox_width // 2
            kernel_half_height = bbox_height // 2
        else:
            kernel_half_width = kernel_half_height = detection_kernel_size // 2
        ...
        x_min = max(x - kernel_half_width, 0)
        x_max = min(x + kernel_half_width, image.shape[1] - 1)
        y_min = max(y - kernel_half_height, 0)
        y_max = min(y + kernel_half_height, image.shape[0] - 1)

        cv2.rectangle(overlay, (x_min, y_min), (x_max, y_max), (0, 255, 0), -1)

        # Draw the text just above the bounding box or kernel area
        text_x, text_y = x_min, y_min - 10
        cv2.putText(image, text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, text_color, 2)
